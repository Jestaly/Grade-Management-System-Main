...
Public Class AddQuizForm
    ...
    Private Function getQuizRowLastIndex() As Integer
        ...
        Try
            ...
            While connector.reader.Read
                value = Integer.Parse(connector.reader("item_name").ToString.Replace("Q", "").ToString)
            End While
    ...
    Private Sub maxScoreButton_Click(sender As Object, e As EventArgs) Handles maxScoreButton.Click
        ...
        Dim itemName = "Q" & (getQuizRowLastIndex() + 1)
        Dim classID As String = LoginForgot.gradingSheet.classChooseBox.Text
        Try
            connector.connect.Open()
            connector.query = "INSERT INTO item VALUES('" & itemID & "','" & itemName & "','Quiz'," & maxScore & ",'" & GradingSheet.getTerm & "','" & classID & "');"
            connector.command.Connection = connector.connect
            connector.command.CommandText = connector.query
        ...
        Catch ex As MySqlException
            connector.connect.Close()
            MessageBox.Show("Database Error maxScoreButton_Click")
        End Try
